...
const locJson = document.querySelector("#locJson").value;
...
function listenforBusSearches(evt){
    if(evt.target.className.includes("monthinput"))
    {
        console.log("Fired");
        const searchInput = evt.target;
        const suggBox = searchInput.nextElementSibling;
...
function showSuggestions()
{
    ...
    let suggestions = locData.filter(({bus_no}) => {
        return bus_no.match(regex);
    }).map(({bus_no}) => {
        const location_num = bus_no.replace(regex, `<span class="hl">${this.value.toUpperCase()}</span>`);;
        return `<li>${location_num}</li>`;
    }).join("");
